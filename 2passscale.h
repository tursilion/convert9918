...
typedef struct 
{ 
   float_precision *Weights;  // Normalized weights of neighboring pixels
...
AllocContributions (UINT uLineLength, UINT uWindowSize)
{
    ...
    for (UINT u = 0 ; u < uLineLength ; u++) 
    {
        ...
        res->ContribRow[u].Weights = new float_precision[uWindowSize];
    }
    return res; 
} 
...
LineContribType *
C2PassScale<FilterClass>::
CalcContributions (UINT uLineSize, UINT uSrcSize, float_precision dScale)
{ 
    FilterClass CurFilter;

    float_precision dWidth;
    float_precision dFScale = 1.0;
    float_precision dFilterWidth = CurFilter.GetWidth();
    ...
    {   // Scan through line of contributions
        float_precision dCenter = (float_precision)u / dScale;   // Reverse mapping
        ...
        float_precision dTotalWeight = 0.0;  // Zero sum of weights
		int iSrc;
        for (iSrc = iLeft; iSrc <= iRight; iSrc++)
        {   // Calculate weights
            dTotalWeight += (res->ContribRow[u].Weights[iSrc-iLeft] =  
                                dFScale * CurFilter.Filter (dFScale * (dCenter - (float_precision)iSrc))); 
        }
            ...
            LineContribType    *Contrib)
{
    ...
    {   // Loop through row
        ...
        {   // Scan between boundries
            ...
            r += (float)(Contrib->ContribRow[x].Weights[i-iLeft] * (float_precision)(pSrcRow[i*3])); 
            g += (float)(Contrib->ContribRow[x].Weights[i-iLeft] * (float_precision)(pSrcRow[i*3+1])); 
            b += (float)(Contrib->ContribRow[x].Weights[i-iLeft] * (float_precision)(pSrcRow[i*3+2])); 
        } 
...
{ 
    ...
    LineContribType * Contrib = CalcContributions (uResWidth, uSrcWidth, float_precision(uResWidth) / float_precision(uSrcWidth)); 
    ...
    {   // Step through rows
        if (NULL != m_Callback)
        {
            ...
            //
            if (!m_Callback (BYTE(float_precision(u) / float_precision (uResHeight) * 50.0)))
            {
                //
...
{ 
    ...
    {    // Loop through column
        ...
        {   // Scan between boundries
            ...
            r += float(Contrib->ContribRow[y].Weights[i-iLeft] * (float_precision)(*pCurSrc));
            g += float(Contrib->ContribRow[y].Weights[i-iLeft] * (float_precision)(*(pCurSrc+1)));
            b += float(Contrib->ContribRow[y].Weights[i-iLeft] * (float_precision)(*(pCurSrc+2)));
        }
...
{ 
    ...
    // Allocate and calculate the contributions
    LineContribType * Contrib = CalcContributions (uResHeight, uSrcHeight, float_precision(uResHeight) / float_precision(uSrcHeight)); 
    ...
    {   // Step through columns
        if (NULL != m_Callback)
        {
            ...
            // Progress and report callback supplied
            //
            if (!m_Callback (BYTE(float_precision(u) / float_precision (uResWidth) * 50.0) + 50))
            {
                //
                // User wished to abort now
                //
